<template>
  ...
  >
    ...
    >
      <div
        class="ticks-header-title"
        style="font-size: 30px; font-weight: 500; color: #0d0d0d"
      >
        Тикетлар
      </div>
      <div
        ...
        style="
          ...
          margin-top: 0px;
        "
        :class="{ 'xr-tick-active': activeInput }"
      >
        <ticketSearch
          @fiter-event="handleFilterEvent"
          @activeInputSend="setActiveInput"
          :projects="projects"
          :status="status"
        >
        </ticketSearch>
        <div
          class="ticks-header-create-btn"
          style="
            border-radius: 8px;
            background: #3680fa;
            color: white;
        ...
        >
          <nav>
            <router-link to="/create"
              ><span
                style="font-size: 18px; margin-bottom: 2px; margin-right: 4px"
                >+Яратиш</span
              ></router-link
            >
          </nav>
        ...
        <table>
          <thead>
            <tr>
              ...
              <th
                class="tickx-table-ticket"
                style="position: sticky; top: 0px; left: 53px"
              >
                тикет номери
              </th>
              <th class="tickx-table-owner">мурожаатчи</th>
              <th
                class="tickx-table-title"
                style="position: sticky; top: 0px; left: 153px"
              >
                мурожат матни
              </th>
          ...
          <tbody>
            <tr
              ...
              :key="index"
              :class="{
                'thickx-body-tr-grey': index % 2 == 1,
                'thickx-body-tr-white': index % 2 == 0,
              }"
            >
              ...
              <td class="tickx-responsible" v-if="executors(index).length > 0">
                <div class="tick-modal-info-header-avatars-block">
                  <div
                    class="tick-modal-info-header-btns-avatar"
                    v-for="(executor, indx) in executors(index)"
                    :key="executor.staff_id"
                    :style="styleAvatar(indx)"
                  ></div>
                  <div
                    v-if="executors(index).length > 3"
                    class="tick-modal-info-header-btns-avatar"
                    :style="styleAvatar(4)"
                  >
                    <div class="tick-modal-info-header-avatar-number">
                      +{{ executors(index).length - 4 }}
                    </div>
                  </div>
                </div>
              </td>
              <td class="tickx-responsible" v-else style="text-align: center">
                -
              </td>
              <td class="tickx-status-td" style="text-align: center !important">
                <span
                  class="tickx-status"
                  :style="setTicketStatus(item.status)"
                  >{{ item.status }}</span
                >
              </td>
...
export default {
  ...
  methods: {
    ...
    setTicketStatus(status) {
      ...
      return (
        statusStyles[status] || { color: "#357bff", background: "#ebf2ff" }
      );
    },
    executors(index) {
